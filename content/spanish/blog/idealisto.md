---
title: "Presentamos ‘Idealisto’: Tu chatbot de IA para el mercado inmobiliario español"
meta_title: "Presentamos ‘Idealisto’: Tu chatbot de IA para el mercado inmobiliario español"
description: "Presentamos ‘Idealisto’: Tu chatbot de IA para el mercado inmobiliario español"
...
## Cómo funciona Idealisto

Idealisto combina potentes tecnologías para proporcionarte información útil:

**LangGraph para manejo de consultas**

* Usando tecnología avanzada basada en gráficos, LangGraph procesa consultas complejas de manera eficiente y también permite depurar cada paso de la cadena, como nuestros *antiguos sistemas backend*.

* Conecta varios puntos de datos para asegurar que tus preguntas, sin importar cuán específicas sean, se entiendan y se respondan.

**Tavily para la recuperación de datos**

* Tavily busca en el sitio *Idealista.com* para obtener listados actualizados, tendencias del mercado y perspectivas sobre propiedades.

* Esto asegura que obtienes datos precisos y en tiempo real directamente de una de las principales plataformas inmobiliarias de España.

**LLM para Respuestas Inteligentes**

* Aprovechando un modelo de lenguaje grande (LLM), Idealisto genera respuestas completas y conversacionales.

* Sintetiza asesoría legal, análisis del mercado y oportunidades de inversión en respuestas fáciles de entender adaptadas a tus necesidades.

## ¿Cómo puedes replicar Idealisto?

Puedes seguir este tutorial paso a paso y copiar y pegar fragmentos de código para recrear la funcionalidad.

El siguiente gráfico representa la lógica que se ejecuta con todas las consultas:

![Multi-agentic architecture](/images/blog/idealisto/lang_graph.webp)

 1. **graph creation**
>  Este es generalmente el paso final, pero se trata solo de mostrar cómo crear un grafo con LangGraph.
```python
...
**2. context_check**
>  Determina si la consulta necesita que Tavily busque información o si el conocimiento interno del LLM es suficiente para responder.
```python
...
```
**3. reflect_idealista**
>  Una herramienta para revisar la pregunta original del usuario y reformularla si es necesario.
```python
    from pydantic import BaseModel, Field
...
**4. reflect_general**
>  Una herramienta para revisar la respuesta del LLM y explorar otras preguntas similares si es necesario.
```python
    from pydantic import BaseModel, Field
...
**5. web_search**
>  Tavily busca en el sitio web idealista.com.
```python
...
**6. idealisto**
>  La LLM genera la respuesta final utilizando toda la información previa.
```python
...
**🔗 ¡Empieza a usar Idealisto hoy mismo y toma el control de tu viaje inmobiliario!**
